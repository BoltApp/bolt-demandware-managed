...
function initExport(context) {
    ...
    var chunkSize = context.chunkSize !== null ? parseInt(context.chunkSize, 10) : -1;
    var path;
 ...
 * Get product data
 * @param {dw.catalog.Product} product product to process
 * @returns {Array} product data object
 */
 ...
 */
function getMasterProductID(product) { // eslint-disable-line no-unused-vars
    var productId = '';
...
function getImages(product) {
    ...
    images.additionalImages = getAdditionalImages(imageProduct, viewTypes);
    return images;
}
 ...
 */
function getAdditionalImages(product, viewTypes) {
    var images = [];
